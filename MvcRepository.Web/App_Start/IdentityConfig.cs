...
namespace MvcRepository.Web
{
    ...
    public class AccountManager : UserManager<Account>
    {
        ...
        {
            var manager = new AccountManager(new UserStore<Account>(context.Get<AppDbContext>()));
            // 設定使用者名稱的驗證邏輯
            ...
            // 您可以撰寫專屬提供者，並將它外掛到這裡。
            manager.RegisterTwoFactorProvider("電話代碼", new PhoneNumberTokenProvider<Account>
            {
                MessageFormat = "您的安全碼為 {0}"
            });
            manager.RegisterTwoFactorProvider("電子郵件代碼", new EmailTokenProvider<Account>
            {
                Subject = "安全碼",
                BodyFormat = "您的安全碼為 {0}"
            });
